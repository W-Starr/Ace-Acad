...
export default function QuizHomeScreen() {
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      try {
        const response = await API.get('/quizzes/home');
        const { tabs, quizzes, retryQuiz } = response.data;
        setTabs(tabs);
        setQuizzes(quizzes);
        setRetryQuiz(retryQuiz);
      } catch (error) {
  ...
  const filteredQuizzes = quizzes.filter(q => {
    ...
    return q.type.toLowerCase() === activeTab.toLowerCase().replace('!', '');
  });
